...
Issue IDs are `<prefix>-<suffix>` where the prefix comes from `issue_prefix` in `.beads/config.yaml`. Always pass full IDs; there are no bare `#42` numbers.
...
Beads writes to a local database, so creating and closing issues is **not** a git operation and needs no push authority. If this repo has a git remote and Dolt sync configured, syncing is a separate, explicit step — follow the repo's git policy in `CLAUDE.md` / `AGENTS.md` before running it.
